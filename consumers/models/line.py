...
class Line:
    ...
    def process_message(self, message):
        ...
        # Based on the message topic, we call the appropriate handler.
        if message.topic() == "com.udacity.clean.stations": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic().startswith("com.udacity.arrival"): # Set the conditional to the arrival topic
            logger.info(f"handling arrival message with key ${message.key}")
